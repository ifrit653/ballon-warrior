...
func _physics_process(delta: float) -> void:
	...
	if player_health == 0 and is_dead:
...
func handle_player_horizontal_wrap(is_dead):
	if not is_dead:
...
func _process(delte : float):
	handle_player_horizontal_wrap(is_dead)
